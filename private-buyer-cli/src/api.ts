...
import {
  ...
  type MiniPrivateBuyerPrivateStateId,
  type MiniPrivateBuyerProviders,
  type DeployedMiniPrivateBuyerContract,
} from './common-types';
...
import {
  ...
  witnesses,
  createPrivateState,
} from '@eddalabs/mini-private-buyer-contract';
...
// Pre-compile the mini-private-buyer contract with ZK circuit assets
...
): Promise<MiniPrivateBuyer.Ledger | null> => {
  assertIsContractAddress(contractAddress);
  logger.info('Checking contract ledger state...');
  const state = await providers.publicDataProvider
    .queryContractState(contractAddress)
    .then((contractState) => (contractState != null ? MiniPrivateBuyer.ledger(contractState.data) : null));
  return state;
};

export const displayLedgerSummary = async (
  providers: MiniPrivateBuyerProviders,
  contract: DeployedMiniPrivateBuyerContract,
): Promise<{ ledger: MiniPrivateBuyer.Ledger | null; contractAddress: string }> => {
  const contractAddress = contract.deployTxData.public.contractAddress;
  const state = await getLedgerState(providers, contractAddress);
  if (state === null) {
    logger.info(`There is no mini-private-buyer contract deployed at ${contractAddress}.`);
  } else {
    const DIV = '──────────────────────────────────────────────────────────────';
    console.log(`
${DIV}
...
): Promise<DeployedMiniPrivateBuyerContract> => {
  const contract = await findDeployedContract(providers, {
    contractAddress,
    compiledContract: miniPrivateBuyerCompiledContract,
    privateStateId: 'miniPrivateBuyerPrivateState',
...
export const balanceOf = async (
  contract: DeployedMiniPrivateBuyerContract,
  owner: MiniPrivateBuyer.Either<MiniPrivateBuyer.ZswapCoinPublicKey, MiniPrivateBuyer.ContractAddress>,
): Promise<FinalizedTxData> => {
...
export const configureProviders = async (walletContext: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<MiniPrivateBuyerCircuits>(contractConfig.zkConfigPath);
  return {
    privateStateProvider: levelPrivateStateProvider<typeof MiniPrivateBuyerPrivateStateId>({
